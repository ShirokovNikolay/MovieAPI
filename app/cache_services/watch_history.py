...
class WatchHistoryCacheService:
    ...
    async def get_watch_history_by_id(
        ...
        cached_watch_history_response = await self.redis_service.get(
            key,
    ...
    async def get_watch_history_list(
        ...
        cached_watch_history_list_response = await self.redis_service.get(
            key,
            WatchHistoryWithMovieResponseList,
        )
    ...
    async def count_user_watch_history(self, user_id: int) -> int:
        ...
        cached_watch_history_response = await self.redis_service.get(key)
        if cached_watch_history_response is not None:
    ...
    async def delete_watch_history_by_id(
        ...
        change_watch_history_count_cache = self.redis_service.incr_by(
            key=watch_history_count_key,
            amount=-1,
        )
        delete_watch_history_item_cache = self.redis_service.delete(watch_history_key)
    ...
    async def delete_user_watch_history(self, user_id: int) -> None:
        ...
        delete_watch_history_count_cache = self.redis_service.delete(
            key=watch_history_count_key,
        )
        await asyncio.gather(
